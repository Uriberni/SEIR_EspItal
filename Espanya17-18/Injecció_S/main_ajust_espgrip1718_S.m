...
fun = @(x)espgrip1718_S(x(1),x(2),x(3),x(4:7));

% limits inferiors variables
llindarslb=[0.7,0.5,0.2];
Sinjlb=[0,0,0,0];
...
disp(x);
disp(Error);
[Error,Nous_agru,Inf,Nous_agru2,peak_day,a,sinj1,S,Nous_set]=espgrip1718_S(x(1),x(2),x(3),x(4:7));

espgrip_1718_S(x(1),x(2),x(3),x(4:7));
%%
figure;
...
title('Recta de regressió Espanya 17-18')
