...
fun = @(x) italgrip1718_S(x(1),x(2),x(3),x(4:7));
...
disp(x);
disp(Error);
[Error,Nous_agru,Inf,Nous_agru2,peak_day,a,sinj1,S,Nous_set]=italgrip1718_S(x(1),x(2),x(3),x(4:7));

italgrip_1718_S(x(1),x(2),x(3),x(4:7));
%%
figure;
...
title('Recta de regressió Itàlia 17-18')
...
if ci_b(2,1) <= 1 && ci_b(2,2) >= 1
    ...
    fprintf('Interval de confiança al 95%% és de %.2f a %.2f\n', ci_b(2,1), ci_b(2,2));
else
